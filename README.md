...
| :small_blue_diamond: | When event is received | When event [myevent] is received with [obj] | :small_orange_diamond: Up to 3 parameters can be received. They can be any value type, except list. | Event runs when a custom event is received by this object. The custom event can be sent by the same script or a script on another object. :small_orange_diamond: Also runs after using reset world codeblock and at the start of a local script. | :small_orange_diamond: Note: names given to input parmeters are local to the event. Use set to codebock to assaign it's value to a variable and make it available to the entire script.
